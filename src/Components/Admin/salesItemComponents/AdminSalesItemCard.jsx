...
export default function AdminSalesItemCard({id , name , desc , price , unit , imgPath}) {
    ...
    const handleDeleteSaleItem = async() => {
      console.log(id);
      const response = await axios.delete("http://localhost:5006/api/salesitems/"+id);
    ...
    <div className="w-72 h-72 flex items-center justify-center border m-1 rounded-lg font-bodyFont">
      <div className="flex flex-col items-center justify-center space-y-1 w-[180px] h-[200px]">
        ...
        <div className="flex items-center justify-center space-x-5 m-3">
          <button className="bg-gray-400 dark:bg-gray-700 text-gray-800 dark:text-gray-200 p-2 text-lg font-bold rounded-full font-bodyFont">Edit</button>
          <button className="bg-red-700 text-lg font-bold font-bodyFont p-2 rounded-full" onClick={handleDeleteSaleItem}>Delete</button>
        </div>
      </div>
    </div>
  );
